...
public class PythonObject {
    ...
    protected List<PythonObject> buildMRO() {
        ...
        mRO.add(this);
        PythonType classType = this.getType();
        if (classType != null) {
            mRO.addAll(classType.getMRO());
        }
        return mRO;
    }
    ...
    public final PythonObject get(String attrName) throws PythonAttributeException {
        if (attrName == null) {
            throw new PythonAttributeException(type, "No attributes with null names");
        }
        ...
        for (PythonObject obj : getMRO()) {
            if (obj.attrs.containsKey(attrName)) {
                PythonObject value = obj.attrs.get(attrName);
                return value; 
            }
        }
    ...
    public final void set(String attrName, PythonObject value) {
        if (attrName == null) { 
            throw new IllegalArgumentException("Attribute name cannott be null"); 
        }
